...
def override_default_baseline():
    copyfile(last_saved_file_name, default_baseline_file_name)
    return

def restore_default_baseline(message_text):
    copyfile(master_baseline_file_name, default_baseline_file_name)
...
def create_master_baseline():
    if path.isfile(master_baseline_file_name) == False:
        ...
        copyfile(master_baseline_file_name, default_baseline_file_name)
        copyfile(master_baseline_file_name, last_saved_file_name)
    return
...
def save_modified_baseline(table, message_text):
    ...
    with open(last_saved_file_name, 'w', newline='') as csvfile:
        ...
        for row in table:
            ...
            csvwriter.writerow(row)
            rowNumber+=1
    #copyfile(last_saved_file_name, file_name)
